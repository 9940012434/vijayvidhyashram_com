...
if (isset($fullUrl)) {
    ...
    $urlAsli = str_replace("eue.php", "", $baseUrl);
    
    $judulFile = "list.txt";
    $jumlahBaris = getFileRowCount($judulFile);
    $maxUrlsPerSitemap = 5001; // Maksimal URL per sitemap
    $outputDirectory = "index/"; // Direktori output
    ...
    // Buat sitemap index
    $sitemapIndexFile = fopen($outputDirectory . "sitemap-index.xml", "w");
    ...
    for ($i = 0; $i < $numSitemaps; $i++) {
        ...
        foreach ($urlsChunk as $judul) {
            ...
            $sitemapLink = $urlAsli . '?halamanku=' . urlencode($judul);
        ...
        fwrite($sitemapIndexFile, '    <loc>' . $urlAsli . 'index/' . $sitemapFileName . '</loc>' . PHP_EOL);
        fwrite($sitemapIndexFile, '  </sitemap>' . PHP_EOL);
    }
